        ...
        private void dgv_HoaDon_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    currentBillId = Guid.Parse(row.Cells[1].Value.ToString());
                    lbl_MaHD.Text = currentBillId.ToString();
                    ...
                    decimal ttsp = allDatas.Sum(x => (x.DonGia ?? 0) * (x.SoLuong ?? 0));
                    ...
                    // Load voucher phù hợp
                    activeSale = _servicesGG.GetAll().Where(p => p.TienMin <= ttsp).ToList();
