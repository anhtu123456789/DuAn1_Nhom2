...
namespace PRL
{
    public partial class FormKhachHang : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            {
                // Kiểm tra dữ liệu
                if (string.IsNullOrWhiteSpace(tbt_makhachhang.Text) ||
                    string.IsNullOrWhiteSpace(tbt_hovaten.Text) ||
                    string.IsNullOrWhiteSpace(tbt_sdt.Text) ||
                    string.IsNullOrWhiteSpace(tbt_diachi.Text) ||
                ...
                // Xác nhận thêm
                DialogResult result = MessageBox.Show("Bạn chắc chắn muốn thêm khách hàng này không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == DialogResult.Yes)
                {
                    string kq = _service.Create(kh);
                    MessageBox.Show(kq);
                    LoadData();
                    reset();
                }
